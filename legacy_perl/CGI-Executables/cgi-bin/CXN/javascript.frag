...
sub js_str_test_popup {
  my $self = shift                                              ;
  my $out=<<END;
<script language="JavaScript">
function UPD(db, depth_str, val_str) {
 upd = window.open("/cgi-bin/Upd", "update_string", 'resizable=yes,scrollbars=yes,status=0,width=250,height=210');
 upd.document.close();
// -->
</script>
END
