...
sub mk_table {
    ...
    my $ret = $table -> gen_tables( $self -> {arr_stack} )            ;
    return $ret ; 
}
   ...
   if ( $ref_thing eq 'HASH' ) {   
      ...
      foreach $key ( @thing_keys ) {
        if (( $key !~ /<STOP\/*>/i ) 
          ...
          my $depth_array ;
          if ( defined $self -> {depth_arr} ) {
          $depth_array = [ @{$self -> {depth_arr}}, @{$self -> {tmparr}} ] ; 
          }
          else { $depth_array = [ @{$self -> {tmparr}} ] }
          my $cell_string 
      ...
      my $key = undef ;
      my @pr_arr = @{$self -> {pr_tmparr}} if defined $self -> {pr_tmparr} ;
      #my $depth_arr = [ @{$self -> {depth_arr}}, @{$self -> {tmparr}},$key ] ; 
      my $depth_array ; 
      if ( defined $self -> {depth_arr} ) {
          $depth_array = [ @{$self -> {depth_arr}}, @{$self -> {tmparr}} ] ;
      }
      elsif(defined $self -> {tmparr}) { 
          $depth_array = [ @{$self -> {tmparr}} ] 
      }

      my $cell_string = join("<BR>", @$thing)                ;
      $cell_string
             .= $self -> cell_string (
                 #$key, $depth_arr, undef, 'test', 'passwd' );
                 $key, $depth_array, 'data' );

      #$cell_string .= "array" if $self -> {debug} ;
      ...
      my @pr_arr = @{$self -> {pr_tmparr}} if defined $self -> {pr_tmparr} ;
      my $depth_array ;
      if (( defined $self -> {depth_arr} ) and ($self -> {tmparr})) {
        $depth_array = [ @{$self -> {depth_arr}}, @{$self -> {tmparr}} ] ;
      }
      elsif ( $self -> {tmparr}) { $depth_array = [ @{$self -> {tmparr}} ] }
...
sub cell_string {
   ...
   my $depth_string ;
   if (( defined $self -> {'base_depth_array'}) 
                and ( defined $depth_array )) {
     $depth_string 
           = join (' ', @{$self -> {'base_depth_array'}}, @{$depth_array} )  ;
   }
   elsif ( defined $depth_array) { 
     $depth_string = join (' ', @{$depth_array} ) 
   }
   else { $depth_string = join (' ', @{$self -> {depth_array}} ) }
   ...
   if ( defined $self -> {upd} ) {
        my $hidden_struct = $self -> {hidden_struct}              ;
        $hidden_struct -> {'depth_string'} = $depth_string        ;
        $hidden_struct -> {'db'} = $self -> {'db'}                ;
        $hidden_struct -> {'secret'} = $self -> {secret}          ;
        $hidden_struct -> {'write'} = $self -> {'write'}          ;
        $hidden_struct -> {'upd'} = $self -> {'upd'}              ;
        $hidden_struct -> {'base_depth_array'}
                                  = $self -> {'base_depth_array'} ;
        $hidden_struct -> {'depth_array'}
                                  = $depth_array ;
        my $hidden_struct_str = $pob -> serialize($hidden_struct) ;
        
        # This works w/ the dialog box
        $cell_string .= " 
    <FORM METHOD=\"POST\" ACTION=\"/cgi-bin/Edit\" ENCTYPE=\"application/x-www-form-urlencoded\" TARGET=\"dialog\">";
        $cell_string .= " 
   <INPUT TYPE=\"hidden\" NAME=\"hidden_struct\" VALUE=\"$hidden_struct_str\">";
        $cell_string .= " 
   <INPUT TYPE=\"submit\" NAME=\"Update Field\" VALUE=\"Edit\" onClick=\"popup_window(\'\',\'dialog\',\'width=550,height=206,scrollbars=yes\')\" >";
        $cell_string .=  "</FORM>";
   }
   return $cell_string
}
...
sub object_cell_string {
   ...
   my $depth_string ;
   if (( defined $self -> {'base_depth_array'})
                and ( defined $arg -> {'depth_array'})) {
     $depth_string
           = join (' ', @{$self -> {'base_depth_array'}}, 
                                               @{$arg -> {'depth_array'}} )  ;
   }
   elsif ( defined $arg -> {'depth_array'}) {
     $depth_string = join (' ', @{$arg -> {'depth_array'}} )
   }
   ...
   if ( $self -> {'upd'} eq 'yes' ) {
        ...
        $hidden_struct -> {'write'} = $self -> {'write'}          ;
        $hidden_struct -> {'upd'} = $self -> {'upd'}            ;
        my $hidden_struct_string = $pob -> serialize($hidden_struct) ;

     if ( $arg -> {'type'} eq 'object' ) { 
        $cell_string .= "
    <FORM METHOD=\"POST\" ACTION=\"/cgi-bin/Edit_Object\" ENCTYPE=\"application/x-www-form-urlencoded\" TARGET=\"dialog\">";
        $cell_string .= "
   <INPUT TYPE=\"hidden\" NAME=\"hidden_struct\" VALUE=\"$hidden_struct_string\">";
        $cell_string .= "
    <INPUT TYPE=\"submit\" NAME=\"Edit_Objects\" VALUE=\"Edit_Objects\" onClick=\"popup_window(\'\',\'dialog\',\'width=325,height=360,scrollbars=yes\')\"></FORM>";
     }
