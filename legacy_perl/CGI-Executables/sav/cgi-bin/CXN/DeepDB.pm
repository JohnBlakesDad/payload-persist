...
sub del {
    ...
    my @these_keys = keys %$hash ;
    my @keys = keys %{$one_up_hash}                       ; 
    print $#keys, "num keys one up hash <BR>" ;
    print "\nhash $hash one_up_arr $#one_up_depth_arr --num keys $#keys-- \n" ; 
    print "xxxy",ref $hash,"yxxx"."<BR>KEYS--".$#these_keys."--KEYS<BR>" ;
    if (( ref $hash ne 'ARRAY') and (  $#these_keys < 0 )) {  
...
sub sync {
my $self  = shift ;
if ( $self -> {DB} -> {write} eq 'yes' ) {
   $self -> {DB} -> sync 
   }
else { print "write not set" }
}
